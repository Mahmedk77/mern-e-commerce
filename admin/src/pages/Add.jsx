    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col gap-3 w-full items-start'>
      ...
      <div>
        ...
        <div className='flex gap-3'>
          {
            data.map((item, idx) => (
              <div key={idx} onClick={() => setSizes(prev => prev.includes(item) ? prev.filter(i => i !== item ) : [...prev, item])} >
                <p onClick={() => setSelectedSizes(prev => prev.includes(item) ? prev.filter(i => i !== item) : [...prev, item])} className={` px-3 py-1 cursor-pointer ${selectedSizes.includes(item) ? 'bg-pink-100': 'bg-slate-200' }`} >{item}</p>
              </div>
            ))
          }
        </div>
